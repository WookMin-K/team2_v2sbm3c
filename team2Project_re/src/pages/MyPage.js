...
const MyPage = () => {
 ...
 const formatPhone = (value) => {
  ...
  if (!numbersOnly.startsWith('010')) return value;
      ...
      <section ref={scrollRef} className="overflow-y-auto scrollable flex-1 mt-16 px-0 pb-40">
       <h2 className="text-3xl text-center font-bold mb-4">내 정보</h2>
        ...
        <div className="w-full max-w-6xl mx-auto flex gap-16">
          {/* 좌측: 프로필(사진 + 버튼) */}
          <div className="flex-none w-60 flex flex-col items-center mt-12">
            <img
              src={loadedUser.profileUrl || DEFAULT_PROFILE}
              alt="프로필"
              className="w-38 h-38 rounded-full object-cover mb-4"
            />
            <button
              onClick={openModal}
                    ...
                    {fieldKey && editField === fieldKey ? (
                     <input
                      ...
                      onChange={e => {
                        ...
                        const newVal = fieldKey === 'phone' ? formatPhone(val) : val;
                        setTempValue(newVal);
                      }}
                      className="border rounded px-3 py-2 w-64"
                      placeholder={fieldKey === 'phone' ? '010-1234-5678' : ''}
                      pattern={fieldKey === 'phone' ? '^010-\\d{4}-\\d{4}$' : undefined}
                      title={fieldKey === 'phone' ? '010-0000-0000 형식으로 입력해 주세요.' : undefined}
                    />
                    ) : (
